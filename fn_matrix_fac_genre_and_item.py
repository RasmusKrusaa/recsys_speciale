...
def traverse_tree(user, current_node: Node, actuals: np.ndarray):
    ...
    if compute_genre:
        ...
        # actuals is inner_user_id x inner_item_id
        answer = round(actuals[current_node.item].loc[user])
...
def compute_minimum_objective(users: [int], node, observations, actuals, item_profiles) -> (int, float):
    ...
    for question in np.setdiff1d(possible_questions, already_asked_questions):
        ...
        for row in like_obs.itertuples():
            l_objective += (int(row.rating) - np.dot(user_like_profile, item_profiles.loc[row.item]).flat[0]) ** 2

        # iterate the user like partition
        dislike_obs = observations[observations.user.isin(d)]
        for row in dislike_obs.itertuples():
            d_objective += (int(row.rating) - np.dot(user_dislike_profile, item_profiles.loc[row.item]).flat[0]) ** 2

        # iterate the user unknown partition
        unknown_obs = observations[observations.user.isin(u)]
        for row in unknown_obs.itertuples():
            u_objective += (int(row.rating) - np.dot(user_unknown_profile, item_profiles.loc[row.item]).flat[0]) ** 2
...
def compute_item_profile(item: int, tree, observations):
    ...
    for row in obs.itertuples():
        ta_i = np.asarray(user_profiles.loc[row.user:row.user])  # t_ai = t(a_i) in the paper
        part1 += np.dot(ta_i.T, ta_i) + np.dot(np.identity(100), lambda_h)  # user profile of each user times it self transposed.
        part2 += int(row.rating) * ta_i  # rating of item times user profile
...
#    item_ids, user_ids = build_item_and_user_id_map(observations)k

    obs = observations[observations.user.isin(users)]
    for row in obs.itertuples():
        item_profile = np.asarray(item_profiles.loc[row.item])
        part1 += np.dot(item_profile, item_profile).T + np.dot(np.identity(100), lambda_h)
...
def greedy_tree_construction(users, user_profile, parent, observations, item_profiles, actuals, depth: int = 0,
    ...
    if best_question[0] == 0:
        print("Chosen item was 0 for some reason...")
        return Node(users, user_profile, None, parent, None, None, None)
...
def partition_users(item: int, users: [int], actuals) -> ([int], [int], [int]):
    ...
    for user in users:
        if compute_genre:
            ...
            # actuals is inner_user_id x raw item_id. We should not need to round this..
            answer = round(actuals[item].loc[user])
...
if __name__ == "__main__":
    ...
    for k in range(1, 6):
        ...
        if train:
            training_observations = pd.read_csv('data/train' + str(k) + '.csv', sep=',').drop('timestamp', axis=1)
            ...
            item_profiles = pd.DataFrame(data=item_profiles, index=unique_items, columns=range(100)).sort_index()
            user_profiles = np.zeros((unique_users.size, 100))
            user_profiles = pd.DataFrame(data=user_profiles, index=unique_users, columns=range(100)).sort_index()
            ...
            if compute_genre:
                ...
                possible_questions = training_observations['item'].value_counts().head(18).index

            actuals = pd.pivot_table(training_observations, index='user', columns='item', values='rating').fillna(value=0)
            ...
            while i != 26 :
                ...
                for user in unique_users:
                    user_profiles.loc[user:user] = traverse_tree(user, greedy_tree, actuals).get_profile().tolist()

                # step 2. Compute new item profiles.
                for item in unique_items:
                    item_profiles.loc[item:item] = compute_item_profile(item, greedy_tree, training_observations)


                predictions = np.dot(user_profiles, item_profiles.T)

                metrics10 = evaluation.evaluation_v2.Metrics2(predictions, actuals.to_numpy(), k=10, metrics='rmse')

                rmse = metrics10.calculate().get('rmse')
                ...
                if i == 26:
                    print('STOPPING!')
                    print('convergence has not been reached in 25 iterations!')

            with open(f'fn_data/rmse_item_train{k}.txt', 'w') as f:
                for item in rmse_list:
                    f.write("%s\n" % item)

            np.savetxt(f'fn_data/fn_item_item_profiles{k}.csv', item_profiles, delimiter=',')
            pickle_tree(greedy_tree, f'item_tree{k}')
        ...
        if test:
            ...
            for user in test_observations.user.unique():
                user_profiles[user] = traverse_tree(user, tree, actuals).get_profile() # carefull here, User_ids[user] may not be correct.
